...
uint8_t compute_init() {
  ...
  if (HAL_TIM_PWM_ConfigChannel(&htim1, &pwm_config, fan_channels[FAN1]) !=
      HAL_OK)
    return -1;
  if (HAL_TIM_PWM_ConfigChannel(&htim8, &pwm_config, fan_channels[FAN2]) !=
      HAL_OK)
    return -1;
...
int16_t compute_get_pack_current() {
  ...
  int16_t high_current_voltage_raw =
      (int16_t)(1000.0f * ((float)raw_high_current * CURRENT_ADC_RESOLUTION));
  high_current_voltage_raw =
      (int16_t)(5000.0f * high_current_voltage_raw / (float)ref_voltage_raw);

  int16_t high_current =
      (high_current_voltage_raw - (1000 * CURRENT_HIGHCHANNEL_OFFSET)) *
      (1 / 4.0f); //* (HIGHCHANNEL_GAIN/100.0f))/1000;

  int16_t low_current_voltage_raw =
      (int16_t)(1000.0f * ((float)raw_low_current * CURRENT_ADC_RESOLUTION));
  low_current_voltage_raw =
      (int16_t)(5000.0f * low_current_voltage_raw / (float)ref_voltage_raw);

  int16_t low_current =
      (float)(low_current_voltage_raw - (1000 * CURRENT_LOWCHANNEL_OFFSET)) *
...
void compute_send_cell_data_message(acc_data_t *bmsdata) {
  ...
  cell_data_msg_data.high_cell_id =
      (bmsdata->max_voltage.chipIndex << 4) | bmsdata->max_voltage.cellNum;
  cell_data_msg_data.low_cell_voltage = bmsdata->min_voltage.val;
  cell_data_msg_data.low_cell_id =
