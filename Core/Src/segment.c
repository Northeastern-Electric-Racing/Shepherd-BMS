...
void select_therm(uint8_t therm) {
  ...
  for (int chip = 0; chip < NUM_CHIPS; chip++) {
    ...
    i2c_write_data[chip][2] =
        (therm - 1); // 0-15, will change multiplexer to select thermistor
  }
  serialize_i2c_msg(i2c_write_data, comm_reg_data);
  push_chip_configuration();
  LTC6804_wrcomm(ltc68041, NUM_CHIPS, comm_reg_data);
...
int pull_voltages() {
  ...
  for (uint8_t i = 0; i < NUM_CHIPS; i++) {
    ...
    for (uint8_t j = 0; j < NUM_CELLS_PER_CHIP + 1; j++) {
      ...
      if (raw_voltages[i][j] > (int)(10000 * (MAX_VOLT + 0.5)) ||
          raw_voltages[i][j] < (int)(10000 * (MIN_VOLT - 0.5))) {
    ...
    for (uint8_t c = 0; c < NUM_CHIPS; c++) {
      ...
      if (therm == current_therm) {
        ...
        uint16_t steinhart_input_low =
            10000 * (float)(((float)raw_temp_voltages[c][2]) /
                                (raw_temp_voltages[c][0]) -
                            1);
        uint16_t steinhart_input_high =
            10000 * (float)(((float)raw_temp_voltages[c][2]) /
                                (raw_temp_voltages[c][1]) -
                            1);
        ...
        if (raw_temp_voltages[c][0] == LTC_BAD_READ ||
            ...
            segment_data[corrected_index].thermistor_value[therm + 15] <
                (MIN_CELL_TEMP - 5)) {
          memcpy(segment_data[corrected_index].thermistor_reading,
                 previous_data[c].thermistor_reading,
                 sizeof(segment_data[corrected_index].thermistor_reading));
          memcpy(segment_data[corrected_index].thermistor_value,
                 previous_data[c].thermistor_value,
                 sizeof(segment_data[corrected_index].thermistor_value));
        }
      } else {
        segment_data[corrected_index].thermistor_reading[therm - 1] =
            previous_data[corrected_index].thermistor_reading[therm - 1];
        segment_data[corrected_index].thermistor_reading[therm + 15] =
            previous_data[corrected_index].thermistor_reading[therm + 15];
...
void segment_enable_balancing(bool balance_enable) {
   ...
   */
  static const uint16_t DICHARGE_ALL_COMMAND =
      0xFFFF >> (16 - NUM_CELLS_PER_CHIP);
...
void serialize_i2c_msg(uint8_t data_to_write[][3], uint8_t comm_output[][6]) {
  for (int chip = 0; chip < NUM_CHIPS; chip++) {
    comm_output[chip][0] =
        0x60 | (data_to_write[chip][0] >> 4); /* START + high side of B0 */
    comm_output[chip][1] =
        (data_to_write[chip][0] << 4) | 0x00; /* low side of B0 + ACK */
    comm_output[chip][2] =
        0x00 | (data_to_write[chip][1] >> 4); /* BLANK + high side of B1 */
    comm_output[chip][3] =
        (data_to_write[chip][1] << 4) | 0x00; /* low side of B1 + ACK */
    comm_output[chip][4] =
        0x00 | (data_to_write[chip][2] >> 4); /* BLANK + high side of B2 */
    comm_output[chip][5] =
...
int8_t calc_therm_standard_dev(int16_t avg_temp) {
  ...
  for (uint8_t chip = 0; chip < NUM_CHIPS; chip++) {
    for (uint8_t therm = 17; therm < 28; therm++) {
      uint16_t sum_diff =
          abs(segment_data[chip].thermistor_value[therm] - avg_temp);
    ...
    for (uint8_t c = 0; c < NUM_CHIPS; c++) {
      for (uint8_t therm = 17; therm < 28; therm++) {
        if (abs(segment_data[c].thermistor_reading[therm] -
                previous_data[c].thermistor_reading[therm]) > 5 &&
