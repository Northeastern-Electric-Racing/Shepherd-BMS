...
void log_fault(uint32_t fault_code)
{
    ...
    uint8_t *address;
    eeprom_read_data_address(eeprom_get_index((char*)("FAULTS")), address, 1);

    uint8_t startIndex =  eeprom_data[eeprom_get_index((char*)("FAULTS"))].address;
    uint8_t size = eeprom_data[eepromGetIndex((char*)("FAULTS"))].size;
    ...
    eeprom_write_data_address(*address, &fault, 4);
    /* update first byte of faults partition*/
    eeprom_write_data_address(startIndex, *(address + 4), 1);
} 

void get_faults()
{
    uint8_t* address;
    eeprom_read_data_key("FAULTS", address, 1);

    uint8_t startAddress =  eeprom_data[eepromGetIndex((char*)("FAULTS"))].address;
    uint8_t size = eeprom_data[eepromGetIndex((char*)("FAULTS"))].size;
    ...
    {
        eeprom_read_data_address(*index, &eeprom_faults[currIter], 4);
        currIter++;
